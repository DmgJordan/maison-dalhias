...
Le Bailleur pourra justifier de la propriete de son bien en fournissant les justificatifs demandes par le Preneur.`;
  ...
  y = addWrappedText(
    doc,
    `Le bien est loue pour ${String(booking.occupantsCount)} occupant${booking.occupantsCount > 1 ? 's' : ''}. Le Preneur s'engage expressement a ne pas depasser ce nombre sans autorisation du proprietaire.`,
    margin,
    y,
    contentWidth,
    5
  ...
  y += 5;
  doc.text(`du ${formatDateShort(booking.startDate)} a 16h00`, margin, y);
  y += 5;
  y = addWrappedText(
    doc,
    `au ${formatDateShort(booking.endDate)} a 11h00, date et heure a laquelle le Preneur s'engage a avoir integralement libere le logement.`,
    margin,
    y,
    contentWidth,
    5
  ...
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(10);
  const depositDeadline = new Date(booking.startDate);
  ...
  // Nouvelle page pour les dernieres sections
  if (y > 200) {
    doc.addPage();
    y = 20;
  }
