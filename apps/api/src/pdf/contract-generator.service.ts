...
Le Bailleur pourra justifier de la propri\u00e9t\u00e9 de son bien en fournissant les justificatifs demand\u00e9s par le Preneur.`;
    ...
    y = addWrappedText(
      doc,
      `Le bien est lou\u00e9 pour ${String(data.occupantsCount)} occupant${data.occupantsCount > 1 ? 's' : ''}. Le Preneur s'engage express\u00e9ment \u00e0 ne pas d\u00e9passer ce nombre sans autorisation du propri\u00e9taire.`,
      margin,
      y,
      contentWidth,
      5
    ...
    y += 5;
    doc.text(`du ${formatDateShort(data.startDate)} \u00e0 16h00`, margin, y);
    y += 5;
    y = addWrappedText(
      doc,
      `au ${formatDateShort(data.endDate)} \u00e0 11h00, date et heure \u00e0 laquelle le Preneur s'engage \u00e0 avoir int\u00e9gralement lib\u00e9r\u00e9 le logement.`,
      margin,
      y,
      contentWidth,
      5
    ...
    // Nouvelle page pour les derni\u00e8res sections — section 15 intentionally skipped
    if (y > 200) {
      doc.addPage();
      y = 20;
    }
